...
from Clase_Manejador import Manejador_Lista_Mail
...
#-------------------------------Menú para opciones de incisos 2 al 4 -------------------------------------------    
    ...
    def opcion3():
        ...
        cant=0
        manej=Manejador_Lista_Mail()
        correo=""
        for fila in reader:
            if(fila[0]=="idCuenta"):
                pass
            else:
                correo=fila[0]+'@'+fila[1]+'.'+fila[2]
                if re.match('^[(a-z0-9\_\-\.)]+@[(a-z0-9\_\-\.)]+\.[(a-z)]{2,15}$',correo.lower()):     # ----- Verifica correctitud de correo, si es correcto crea instancia y muestra datos 
                    manej.agregaEmail(correo)                           # *** Agrega a la lista los correos correctos ***

        cant=manej.buscarMailporDom(domi)
        print("La cantidad de correos con dominio " + domi + " es: " + str(cant))
        input("Enter para continuar...")
        os.system('cls')
        archivo.close()
